...
class MovieSimilarities(MRJob):
    ...
    #using the steps method of MRStep to set each map-reduce step
    def steps(self):
        return [
    ...
    def mapper_parse_input(self, key, line):
        # Outputs the following : userID => (movieID, rating)
        (userID, movieID, rating, timestamp) = line.split('::')
        if (userID != 'userId'):  
            yield  userID, (movieID, float(rating))

    def reducer_ratings_by_user(self, user_id, itemRatings):
        # Using userID, group all the pairs of (movieID, rating)
    ...
    def mapper_create_item_pairs(self, user_id, itemRatings):
        # Code to find all the pairs of movies each user has seen and
        # give out each pair with its associated ratings

        # for a user, "combinations" finds every possible pair from the list of movies
        # that this user viewed.
